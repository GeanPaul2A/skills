...
cp "$S/sistema-diseno/plantillas/componentes-base.json" "$DESTINO/inventario/componentes.json"
cp "$S/sistema-diseno/plantillas/plantillas-base.json"  "$DESTINO/inventario/plantillas.json"

# 3 · Derivar los tres niveles de token
python3 "$S/sistema-diseno/scripts/derivar.py" --destino "$DESTINO" >/dev/null
...
python3 "$S/sistema-diseno/scripts/construir.py" --destino "$DESTINO" \
