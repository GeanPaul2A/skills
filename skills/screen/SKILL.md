...
Busca `output/marca.json` y `output/tokens/` en el proyecto. **Si no están, para acá** y ofrece construirlo con
`system-design`. No improvises tokens.

**Y busca `output/domains/<tipo>.json`: es el negocio.** Si no está, la skill `domain` lo define antes de dibujar.

**Lee `output/inventario/componentes.json` y `output/inventario/plantillas.json`**: eso es lo que hay disponible. **No se
...
**Si hay un `output/domains/<tipo>.json`, esto se comprueba solo**: cada dato se cruza contra una entidad y un campo
de ese dominio — DS-P02. **Si no hay dominio, la skill `domain` lo define primero.** Sin dominio, la pantalla
...
Un archivo por pantalla, en `output/screens/<nombre>.json`. **El formato, en `${CLAUDE_SKILL_DIR}/referencias/generacion.md`.**
...
```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/verificar-screen.py --sistema <sistema> --screens <carpeta>
```
...
**Esta skill exige un puente que escriba en el lienzo (MCP de Figma).** Si no hay puente de escritura, no se
dibuja: se dice y se para. El orden de creación es fijo: importar variables → estilos → componentes → **y
recién ahí** las pantallas, cada una dentro de su marco de dispositivo.

**Antes de dibujar, comprueba el puente y el permiso** — `${CLAUDE_PLUGIN_ROOT}/skills/system-design/referencias/puentes.md`.
