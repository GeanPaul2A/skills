...
los datos más largos, la lista vacía y el estado de error — y se recorre con teclado. `test` recorre todo eso
...
**6 · Se mide antes de opinar.** `${CLAUDE_SKILL_DIR}/scripts/test.py` comprueba ocho reglas solo —momentos,
...
```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/test.py --sistema <sistema> --screens <carpeta> \
        --html <sistema>/outputs/pruebas/index.html
```
...
La pantalla (`output/screens/<nombre>.json`) y el dominio (`output/domains/<tipo>.json`). Si no existen, no
hay nada que probar: decirlo.

### Paso 3 · Los cinco momentos

Para cada flujo, recorrer **entrada → decisión → éxito → error → salida**. El detalle de cada momento, en
`${CLAUDE_PLUGIN_ROOT}/skills/screen/referencias/patrones.md`. Si un momento no existe, es un fallo — DS-P03.
...
Reporte HTML: `outputs/pruebas/<nombre>.html` — cada momento y estado, con lo que pasó y lo que falta.
...
| Archivo | Cuándo |
|---|---|
| `${CLAUDE_SKILL_DIR}/scripts/test.py` | **Siempre, primero.** Las ocho comprobaciones automáticas |
| `${CLAUDE_PLUGIN_ROOT}/skills/screen/referencias/patrones.md` | Los cinco momentos y cómo se declaran |
