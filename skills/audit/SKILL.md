...
se construye. `audit` evalúa un sistema o unas pantallas **ya existentes** y dice **qué tan sano está** y **qué
arreglar primero**.
...
`output/marca.json`, `output/tokens/`, `output/inventario/` y `output/screens/`. Si no hay sistema, no hay nada que auditar: decirlo.

### Paso 2 · Correr lo automático

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/system-design/scripts/verificar.py --destino <destino>
python3 ${CLAUDE_PLUGIN_ROOT}/skills/screen/scripts/verificar-screen.py --sistema <s> --screens <p>
```
...
```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/audit.py --destino <destino> --screens <carpeta> \
        --html <destino>/outputs/audit/index.html
```
...
**Tres acciones, por impacto.** El informe completo como **HTML**: `outputs/audit/index.html`.
...
| Archivo | Cuándo |
|---|---|
| `${CLAUDE_SKILL_DIR}/scripts/audit.py` | **Siempre.** Calcula el score y la cobertura de las reglas |
