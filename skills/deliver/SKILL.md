...
```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/deliver.py --destino <destino> --iniciar
```

Crea `output/delivery/estructura.json` con las **siete páginas del producto** y las **seis del sistema**, ya
nombradas y en orden, más `output/motion.json` y las carpetas de recursos. **No inventa contenido: crea los
...
```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/icons.py --catalogo
python3 ${CLAUDE_SKILL_DIR}/scripts/icons.py --destino <destino> --plataforma ios --uso barra
```
...
```
output/recursos/iconos/      solo .svg, del catálogo
output/recursos/imagenes/    solo .webp o .avif
```
...
En `output/motion.json`, una entrada por animación con **los cinco datos**:
...
```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/deliver.py --destino <destino>
```
...
| **De cada entrega** | `output/delivery/versiones.json` → `entregas[]` | Un cambio en las pantallas de esa funcionalidad |
...
| **PNG donde va una foto** | `output/recursos/imagenes/*.png` | WebP o AVIF: 87 % menos — DS-H04 |
