...
# -*- coding: utf-8 -*-
"""
deliver.py — comprueba el paquete de entrega: estructura, recursos y movimiento.

    python3 deliver.py --destino <carpeta>
    python3 deliver.py --destino <carpeta> --iniciar        crea el esqueleto
    python3 deliver.py --destino <carpeta> --romper DS-F10  prueba el verificador
...
class Entrega:
    def __init__(self, destino):
        ...
        self.estructura = self._json("delivery/estructura.json")
        self.motion = self._json("motion.json")
        self.versiones = self._json("delivery/versiones.json")
    ...
    def animaciones(self):
        return (self.motion or {}).get("animaciones", {})
...
def h01_siete_paginas(e):
    ...
    if not e.estructura:
        return r.saltar("no hay delivery/estructura.json — córrelo con --iniciar")
...
def h08_nada_se_borra(e):
    ...
    if not e.estructura:
        return r.saltar("no hay delivery/estructura.json")
...
def h07_version_por_hito(e):
    ...
    if not e.versiones:
        return r.saltar("no hay delivery/versiones.json — todavía no se cerró ningún ciclo")
...
def h05_cinco_datos(e):
    ...
    anim = e.animaciones()
    if not anim:
        return r.saltar("no hay motion.json — el sistema todavía no declara animación")
...
def h10_entregas_atadas(e):
    ...
    entregas = e.entregas()
    if not entregas:
        return r.saltar("no hay entregas cerradas todavía en delivery/versiones.json")
...
def romper(e, regla):
    ...
    daños = {
        ...
        if e.estructura else sys.exit("hace falta delivery/estructura.json para romper DS-H01"),
        ...
        "DS-H05": lambda: e.__setattr__(
            "motion", {"animaciones": {"colada": {"clase": "adorno", "duracion": "200ms"}}}),
        "DS-H06": lambda: e.__setattr__(
            "motion", {"animaciones": {"colada": {
                "clase": "adorno", "duracion": "200ms", "curva": "ease-out",
