...
# -*- coding: utf-8 -*-
"""
auditar.py — mide el estado de un sistema ya construido y calcula el score.

    python3 auditar.py --destino <sistema> [--pantallas <carpeta>]
    python3 auditar.py --destino <sistema> --html salidas/auditoria/index.html
    python3 auditar.py --destino <sistema> --romper DS-T07
...
class Auditoria:
    def __init__(self, destino, pantallas=None):
        ...
        d = pathlib.Path(pantallas) if pantallas else self.raiz / "pantallas"
...
def m1_nombres(a):
    ...
    for grupo, patron, que in ((a.sem, CONVENIO_TOKEN, "rol semántico"),
                               ...
                               (a.pantallas, re.compile(r"^[a-z0-9]+(?:[-_][a-z0-9]+)*$"),
                                "pantalla")):
        for nombre in grupo:
...
def m4_inventario(a):
    ...
    for nombre, p in a.pantallas.items():
...
<p class=sub>{len(a.piezas())} componentes · {len(a.pantallas)} pantallas ·
...
def romper(a, regla):
    ...
    daños = {
        ...
        "DS-C01": lambda: (a.pantallas or {"x": {}}).setdefault(
            next(iter(a.pantallas), "pantalla-de-prueba"), {}).setdefault(
...
def main():
    ...
    ap.add_argument("--pantallas", help="carpeta de pantallas, si no es <destino>/pantallas")
    ...
    a = Auditoria(a_.destino, a_.pantallas)
    if a_.romper:
