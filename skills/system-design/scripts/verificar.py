...
# El cálculo de contraste, el resultado de una comprobación y el veredicto de --romper
# viven en `lib/comun.py`. Estaban copiados en tres guiones, y la regla 3 de la skill
# dice por qué eso no se sostiene: «si un valor aparece en dos archivos, uno de los dos
# va a quedar viejo».
...
class Sistema:
    def __init__(self, destino):
        ...
        # una vez, en vez de en cada comprobación: una que se olvide de saltarlas recibe
        # una cadena donde espera un objeto y revienta con un error que no dice nada.
        self.componentes = self._tabla("inventario/componentes.json", "componentes")
        self.patrones = self._tabla("inventario/patrones.json", "patrones")
        self.plantillas = self._tabla("inventario/plantillas.json", "plantillas")
    ...
    def _tabla(self, rel, clave):
        return tabla(self._json(rel), clave)
...
def b06_hover(s):
    ...
    if not s.componentes:
        return r.saltar("no hay inventario")
    # `web` y `desktop` tienen puntero; `ios` y `android`, no. Un producto que declara
    # alguna con puntero puede usar hover legítimamente.
    if set(s.plataformas) - {"ios", "android"}:
    ...
    for nombre, c in s.componentes.items():
        ...
        if any(e in ("hover", "sobrevuelo") for e in estados):
            r.mal(f"{nombre}: declara 'hover' y el producto es solo táctil")
        else:
            r.ok()
    return r
...
# ═══ Eje B · Composición · `11-composicion` ══════════════════════════════════

def _catalogo_iconos():
    f = pathlib.Path(__file__).resolve().parents[3] / "recursos/iconos.json"
    return json.loads(f.read_text(encoding="utf-8")) if f.exists() else None


# DS-C12 — los rangos Unicode donde viven los emoticones y pictogramas.
RANGOS_EMOJI = [(0x1F300, 0x1FAFF), (0x2600, 0x27BF), (0x1F000, 0x1F0FF),
                (0xFE0F, 0xFE0F), (0x1F1E6, 0x1F1FF)]


def es_emoji(car):
    p = ord(car)
    return any(a <= p <= b for a, b in RANGOS_EMOJI)


def b16_tamano_icono(s):
    """DS-C11 · el icono dentro de un componente sale de la tabla de su plataforma.

    Es el error más frecuente y el más visible: un icono de 24 dentro de un campo de 44
    de alto se ve gigante, y el mismo icono en una barra se ve bien. No es el icono — es
    la relación con lo que lo rodea, y por eso el tamaño depende del USO, no del gusto.
    """
    r = R("DS-C11", "todo icono declara su uso y toma el tamaño de la tabla")
    cat = _catalogo_iconos()
    if not cat:
        return r.saltar("no hay recursos/iconos.json en el plugin")
    conicono = {n: c for n, c in s.componentes.items()
                if isinstance(c, dict) and (c.get("icono") or "icono" in n)}
    if not conicono:
        return r.saltar("ningún componente del inventario declara icono")
    usos = set(cat["_tamanos"]["ios"])
    for nombre, c in conicono.items():
        ic = c.get("icono")
        if not isinstance(ic, dict):
            r.mal(f"{nombre}: lleva icono y no declara su uso — va «icono»: "
                  f"{{\"uso\": \"barra|control|campo|linea|grande\"}}")
            continue
        uso = ic.get("uso")
        if uso not in usos:
            r.mal(f"{nombre}: uso «{uso}» no está en la tabla ({', '.join(sorted(usos))})")
            continue
        for plat in s.plataformas:
            tabla = cat["_tamanos"].get(plat)
            if not tabla:
                continue
            declarado = (ic.get("tamanos") or {}).get(plat)
            if declarado is not None and declarado != tabla[uso]:
                r.mal(f"{nombre}: en {plat} declara {declarado} px y la tabla dice "
                      f"{tabla[uso]} para uso «{uso}»")
            else:
                r.ok()
    return r


def b17_sin_emoticones(s):
    """DS-C12 · ningún emoticón hace de icono de interfaz.

    Un emoticón se parece a un icono y falla en las cuatro cosas que un icono hace: no
    hereda el color, cambia de forma entre versiones del sistema operativo, se ve distinto
    en cada plataforma, y el lector de pantalla lo lee con un nombre que nadie eligió.
    """
    r = R("DS-C12", "ningún emoticón hace de icono en el inventario")
    piezas = {n: c for n, c in s.componentes.items() if isinstance(c, dict)}
    if not piezas:
        return r.saltar("no hay componentes que revisar")
    for nombre, c in piezas.items():
        texto = json.dumps(c, ensure_ascii=False)
        malos = sorted({ch for ch in texto if es_emoji(ch)})
        if malos:
            r.mal(f"{nombre}: usa {' '.join(malos)} — va un icono del catálogo, "
                  f"que sí hereda color y lleva texto alternativo")
        else:
            r.ok()
    return r
...
def b18_anidacion(s):
    ...
    El tope de profundidad no es burocracia: un botón dentro de un chip dentro de una
    tarjeta dentro de una lista son cuatro rellenos acumulados, el contenido queda a 64 px
    del borde, y **nadie decidió eso** — se sumó solo.
    """
    r = R("DS-C14", "lo que contiene a otros declara qué admite y a qué profundidad")
    contenedores = {n: c for n, c in s.componentes.items()
                    if isinstance(c, dict) and c.get("admite")}
    if not contenedores:
        return r.saltar("ningún componente declara «admite»")
    declarados = set(s.componentes)
    for nombre, c in contenedores.items():
        for hijo in c["admite"]:
            if hijo not in declarados:
                r.mal(f"{nombre}: admite «{hijo}», que no existe en el inventario")
            else:
                r.ok()
        p = c.get("profundidad")
        if not isinstance(p, int) or not 1 <= p <= 3:
            r.mal(f"{nombre}: profundidad «{p}» — va un entero de 1 a 3. Más de 3 es "
                  f"relleno acumulado que nadie decidió")
        else:
            r.ok()
    return r
...
def romper(s, regla):
    ...
    daños = {
        ...
        "DS-C10": lambda: (s.__setattr__("plataformas", ["ios"]),
        ...
        "DS-C14": lambda: next(
            c for c in s.componentes.values()
            if isinstance(c, dict) and c.get("admite")).__setitem__("profundidad", 9),
