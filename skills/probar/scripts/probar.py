...
# -*- coding: utf-8 -*-
"""
probar.py — prueba pantallas y flujos: momentos, estados, valores límite y accesibilidad.

    python3 probar.py --sistema <sistema> --pantallas <carpeta>
    python3 probar.py --sistema <s> --pantallas <p> --html salidas/pruebas/index.html
...
class Prueba:
    def __init__(self, sistema, pantallas):
        ...
        self.pantallas = {p.stem: json.loads(p.read_text(encoding="utf-8"))
...
def c03_estados(p):
    ...
    for nombre, pan in p.pantallas.items():
...
def l03_eje_del_texto(p):
    ...
    usadas = {pan.get("plantilla") for pan in p.pantallas.values()}
    if not usadas:
...
def a07_teclado(p):
    ...
    if not interactivos:
        return r
    if not css:
        return r.saltar("no hay salidas/sistema.css: el foco declarado no se puede confirmar")
...
def a08_zoom(p):
    ...
    galeria = p.raiz / "salidas/galeria"
    archivos = sorted(galeria.glob("*.html")) if galeria.is_dir() else []
    if not archivos:
        return r.saltar("no hay salidas/galeria: corré construir.py --salidas galeria")
    for f in archivos:
...
<p class=sub>{len(p.pantallas)} pantallas · {len(p.patrones)} patrones</p>
...
SUCIO = "salidas/galeria/prueba-a08.html"
...
def romper(p, regla):
    ...
    def primera_pantalla():
        ...
        return next(iter(p.pantallas.values()))
    ...
    daños = {
        ...
        "DS-A08": lambda: (p.raiz / "salidas/galeria").mkdir(parents=True, exist_ok=True) or
...
def main():
    ...
    p = Prueba(a.sistema, a.pantallas)
    if a.romper:
