...
| **`DS-F02`** | Ningún texto traducible usa tamaño fijo | OBLIGATORIO | Automática | `test.py` ✓ | Libro 1 · 3 |
| **`DS-F03`** | Cuerpo ≥ 16 px, interlineado entre 1.4 y 1.6 | OBLIGATORIO | Automática | `verificar-screen.py` · `verificar.py` ✓ | Libro 1 · 7 |
...
| **`DS-T04`** | Un solo convenio de nombres en todo el sistema | OBLIGATORIO | Automática | `audit.py` · `deliver.py` · `verificar.py` ✓ | Libro 1 · 6 |
...
| **`DS-T07`** | Ningún valor en crudo en una pantalla | OBLIGATORIO | Automática | `audit.py` · `verificar-screen.py` · `verificar.py` ✓ | Extensión G1 |
...
| **`DS-C09`** | Se agrupa como variantes solo lo que difiere de forma limitada | RECOMENDADO | Manual | `audit.py` ✓ | Libro 2 · 7 |
| **`DS-C10`** | El estado `hover` no se declara para móvil | RECOMENDADO | Automática | `verificar.py` ✓ | Libro 2 · 8 |
| **`DS-C11`** | El icono dentro de un componente sale de la tabla de tamaños de su plataforma | OBLIGATORIO | Automática | `icons.py` · `verificar.py` ✓ | Extensión G8 |
...
| **`DS-L03`** | Ningún contenedor de texto usa `Fixed` en el eje del texto | OBLIGATORIO | Asistida | `test.py` ✓ | Libro 1 · 3 |
| **`DS-L04`** | La estructura se construye con marcos, no con grupos | OBLIGATORIO | Asistida | — | Libro 2 · 3 |
| **`DS-L05`** | Se diseña móvil primero | OBLIGATORIO | Manual | `test.py` | Libro 2 · 8 |
| **`DS-L06`** | Toda pantalla se prueba con los valores más largos y más cortos de su tabla | OBLIGATORIO | Automática | `test.py` · `verificar-screen.py` ✓ | Libro 2 · 4 · Extensión G2 |
...
| Identificador | Enunciado | Nivel | Método | Guion | Origen |
|---|---|---|---|---|---|
| **`DS-P01`** | Todo patrón declara dominio, tablas y reglas en el inventario | OBLIGATORIO | Automática | `inyectar.py` · `verificar-screen.py` · `verificar.py` ✓ | Extensión G2 |
| **`DS-P02`** | Ningún dato se muestra sin una columna que lo respalde | OBLIGATORIO | Automática | `inyectar.py` · `verificar-screen.py` · `verificar.py` ✓ | Libro 2 · 4 · Extensión G2 |
| **`DS-P03`** | Todo patrón enumera sus estados, y al menos uno es un fallo | OBLIGATORIO | Automática | `test.py` · `verificar-screen.py` · `verificar.py` ✓ | Libro 1 · 8 · Libro 2 · 9 |
| **`DS-P04`** | Lo que viene de otro dominio declara qué se muestra si no llega | OBLIGATORIO | Automática | `verificar-screen.py` ✓ | Extensión G2 |
| **`DS-P05`** | Ninguna superficie continua no textual es el único portador de información necesaria | OBLIGATORIO | Manual | `test.py` | Extensión G4 · Libro 1 · 7 |
...
| Identificador | Enunciado | Nivel | Método | Guion | Origen |
|---|---|---|---|---|---|
| **`DS-A01`** | Nivel objetivo WCAG 2.1 AA; AAA en lo crítico | OBLIGATORIO | Declarativa | `audit.py` | Libro 1 · 7 |
| **`DS-A02`** | Texto 4.5:1, foco 3:1, comprobado al definir el token | OBLIGATORIO | Automática | `verificar.py` ✓ | Libro 1 · 7 |
| **`DS-A03`** | Ninguna información se comunica solo con color | OBLIGATORIO | Manual | `test.py` | Libro 1 · 7 |
...
| **`DS-A05`** | Un solo H1 por pantalla, con jerarquía descendente | OBLIGATORIO | Automática | `test.py` · `verificar-screen.py` ✓ | Libro 1 · 7 |
| **`DS-A06`** | Todo icono con significado lleva texto alternativo de función | OBLIGATORIO | Automática | `verificar.py` ✓ | Libro 1 · 7 |
| **`DS-A07`** | Lo que se puede con ratón se puede con teclado, con foco visible | OBLIGATORIO | Asistida | `test.py` · `verificar-screen.py` · `verificar.py` ✓ | Libro 1 · 7 |
| **`DS-A08`** | Toda pantalla se revisa al 200 % de texto | OBLIGATORIO | Asistida | `test.py` ✓ | Libro 1 · 7 |
...
| **`DS-A11`** | Se revisa en un dispositivo de gama baja | RECOMENDADO | Manual | `test.py` | Libro 1 · 7 |
| **`DS-A12`** | `axe-core` en la tubería cuando exista la aplicación | RECOMENDADO | Declarativa | `test.py` | Libro 1 · 7 |
| **`DS-A13`** | Toda pantalla tiene un solo foco visual primario, y se declara cuál | OBLIGATORIO | Automática | `verificar-screen.py` ✓ | Extensión G10 |
...
| **`DS-H05`** | Toda animación se entrega con sus cinco datos | OBLIGATORIO | Automática | `deliver.py` ✓ | Libro 1 · 8 |
| **`DS-H06`** | El movimiento se anima con `transform`, no con posición | OBLIGATORIO | Automática | `deliver.py` ✓ | Libro 1 · 8 |
| **`DS-H07`** | Versión manual al cerrar un ciclo, con nombre por hito | RECOMENDADO | Automática | `deliver.py` ✓ | Libro 1 · 1 |
| **`DS-H08`** | Nada se borra: lo descartado va a la página de archivo | RECOMENDADO | Automática | `deliver.py` ✓ | Libro 1 · 4 |
| **`DS-H09`** | El sistema declara su versión, y es semántica | OBLIGATORIO | Automática | `deliver.py` ✓ | Extensión G7 |
| **`DS-H10`** | Toda entrega declara contra qué versión del sistema se dibujó | OBLIGATORIO | Automática | `deliver.py` ✓ | Extensión G7 |
