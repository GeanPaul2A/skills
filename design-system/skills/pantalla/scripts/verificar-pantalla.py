...
# -*- coding: utf-8 -*-
"""
verificar-pantalla.py — comprueba pantallas declaradas contra el sistema y contra el modelo.

    python3 verificar-pantalla.py --sistema <carpeta> --pantallas <carpeta>
    python3 verificar-pantalla.py --sistema <s> --pantallas <p> --romper DS-P02
...
class Contexto:
    def __init__(self, sistema, pantallas):
        ...
        self.componentes = (self._j(self.sis / "inventario" / "componentes.json")
                            or {}).get("componentes", {})
        self.plantillas = (self._j(self.sis / "inventario" / "plantillas.json")
                           or {}).get("plantillas", {})
        self.patrones = (self._j(self.sis / "inventario" / "patrones.json")
                         or {}).get("patrones", {})
        ...
        if not self.dir.exists():
            sys.exit(f"no encuentro pantallas en {self.dir} — la carpeta no existe. "
                     f"Declaralas con la skill pantalla antes de verificar")

        self.pantallas = {}
        for f in sorted(self.dir.glob("*.json")):
            self.pantallas[f.stem] = self._j(f)
...
def p03_zonas(c):
    ...
    if not (c.pantallas and c.plantillas):
...
def main():
    ...
    ap.add_argument("--pantallas", required=True, help="carpeta con las pantallas declaradas")
    ap.add_argument("--regla")
    ap.add_argument("--romper", metavar="DS-XXX")
    a = ap.parse_args()

    c = Contexto(a.sistema, a.pantallas)
    if a.romper:
        print(f"⚠  error inyectado a propósito en {romper(c, a.romper)} — "
              f"se espera que la comprobación FALLE\n")

    print(f"{len(c.pantallas)} pantallas · sistema en {c.sis.name}\n")
    ok = mal = 0
    saltadas = []
