...
> Base.** Este documento es el que origina la estructura: se escribe **antes** que la base de conocimiento y explica por qué
...
>
> **Agnóstico de negocio:** esta base de conocimiento no describe ningún producto. Todo lo que aquí se afirma vale igual para
...
3. [Qué es distinto en esta base de conocimiento](#3--qué-es-distinto-en-esta-base-de-conocimiento)
...
|---|---|---|
| **Design Beyond Limits with Figma** — Simon Jun | Sistemas de diseño, **tokens**, accesibilidad, entrega a desarrollo, colaboración | **El libro del sistema.** Columna vertebral de la base de conocimiento |
| **Designing and Prototyping Interfaces with Figma, tercera edición** | Rejillas, color y tipografía, **Auto Layout**, componentes y variantes, variables, prototipado | **El libro del oficio.** Cómo se construye cada pieza |

**No se solapan: se completan.** El primero dice **cómo se estructura un sistema**; el segundo, **cómo se
dibuja lo que el sistema declara**. Una base de conocimiento que solo tuviera el primero produciría un sistema imposible de
...
| Cap. | Contenido | Utilidad para la base de conocimiento |
|---|---|---|
...
| **6** | **Tokens de diseño** | **Máxima.** Es el capítulo que gobierna toda la base de conocimiento |
...
| 11 | Exportación y entrega | **Alta.** Junto con el capítulo 8 del libro 1 |
...
## 3 · Qué es distinto en esta base de conocimiento

**Los dos libros asumen un equipo de diseño humano.** Esta base de conocimiento asume un agente que lee, y eso cambia qué hay
que escribir.

| Lo que los libros asumen | Lo que asume esta base de conocimiento | Consecuencia para la base de conocimiento |
|---|---|---|
...
**Cada extensión de la base de conocimiento nace de uno de estos, detectado leyendo.**

| # | Vacío | Evidencia en el libro | Extensión que lo llena |
|---|---|---|---|
| **G1** | **No dice cómo verificar** que un diseño respeta el sistema. Todo el control es humano — revisiones mensuales, analíticas de *detach*, gobierno | Libro 1, capítulo 5: el control son reuniones y métricas de uso, nunca una comprobación automática | `rules/` + el verificador |
...
| **G3** | **No define un contrato legible por máquina** de los componentes. El libro exige declarar variantes, tamaños y estados, pero como propiedades de Figma | Libro 1, capítulo 5: las propiedades son `◆ Variant`, `○ Toggle`, `@ Content` — dentro de Figma | `inventario/` en JSON |
...
| **G5** | **Da por sentado que hay identidad de marca.** Ambos parten de que el color y la letra ya se decidieron | Libro 2, capítulo 5 enseña a *aplicar* color y tipografía, no a *elegirlos* | `foundations/` con el método de elección |
| **G6** | **No trata el multi-país como dato.** El tema se plantea como marca (theming), no como moneda, idioma y formato variables | Libro 1, capítulo 6: los temas son variantes de marca | `rules/` — formato de número, moneda y largo de texto |

---

## 5 · La estructura que resulta

**El libro 1 ofrece dos arquitecturas de componentes y la base de conocimiento toma la segunda**, que es la que el propio autor
...
> entre niveles de complejidad."* — Libro 1, capítulo 5
...
| `02-tokens/` | `[Libro 1, capítulo 6]` | Los **tres niveles**, nomenclatura, alias, la regla de las tres apariciones |
| `03-components/` | `[Libro 1, capítulo 5]` · `[Libro 2, capítulo 7]` | Arquitectura, propiedades, variantes, tamaños, **estados** |
| `04-auto-layout/` | `[Libro 2, capítulo 6]` | Lo que en código es *flexbox*: dirección, espacio, relleno, ajuste |
| `05-patterns/` | `[Extensión G2, G4]` | Cómo se declara un patrón — **dominio, tablas, reglas y estados** |
| `06-accessibility/` | `[Libro 1, capítulo 7]` | Contraste, teclado, lector de pantalla, WCAG |
| `07-handoff/` | `[Libro 1, capítulos 4 y 8]` | Estructura de archivo, modo de desarrollo, exportación, variables por plataforma |
| `08-figma-bridge/` | `[Libro 1, capítulo 6]` · `[Libro 2, capítulo 13]` | Variables contra Token Studio, importación y exportación |
| `09-rules/` | `[Extensión G1, G6]` | **Reglas `DS-xxx`** obligatorias, y qué verifica cada una |
...
**Igual que `DB-xxx` en la base de conocimiento de modelado.** Una regla con número se puede citar desde el verificador, desde
...
**Se registra a propósito:** una base de conocimiento que afirma cosas de un capítulo no leído es exactamente el hallazgo
...
capítulo por capítulo qué sección sostiene cada uno, para que ninguna afirmación de la base de conocimiento quede sin respaldo.
...
**Libro 2, capítulo 4** — y describe el error más común al empezar una pantalla:
...
**El libro 2 documenta el servidor MCP de Figma, y nombra a Claude Code entre sus clientes** `[Libro 2, capítulo 11]`.
...
**Se deja el párrafo anterior en pie, tachado por este.** Una base de conocimiento que borra su conclusión vieja pierde la
...
**Y hay una consecuencia práctica que sí es segura** `[Libro 2, capítulo 11]`:
